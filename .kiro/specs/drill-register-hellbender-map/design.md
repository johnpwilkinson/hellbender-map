...
|---|---|---|
| `docs/drills/drill-register-hellbender-map.md` | Owns | The drill marker document; the sole artifact of this feature. |

## Boundary Commitments

| Commitment | Detail |
|---|---|
| Drill isolation | `src/drill/drill-register-hellbender-map` MUST NOT import `src/components` — the drill owns no source modules; this guards the reserved drill source path. |
| Product isolation | `src/components` MUST NOT import `src/drill/drill-register-hellbender-map` — product code never depends on a throwaway drill marker. |
...
- **Docs-only implementation** — the implementer creates exactly one file,
  `docs/drills/drill-register-hellbender-map.md`, and does not modify `package.json` or any
  source file. The spec's own process artifacts (`.kiro/specs/drill-register-hellbender-map/`,
...
- **Inert boundary rules by construction** — the two table commitments
  ...
  `src/drill/drill-register-hellbender-map`, which this drill never creates. They are meant to
...
- **Throwaway grade** — the marker exists to prove the runway; deleting it
  (and the `sdd-drill-register-hellbender-map-*` rules) after the drill PR merges is
  sanctioned.

## Concrete Shape

`docs/drills/drill-register-hellbender-map.md` renders as a heading line `# drill-register-hellbender-map` followed
by one short paragraph. An acceptable rendering (any wording that states
marker + safe-to-delete is conformant):

    # drill-register-hellbender-map
